...
export async function fetchProviderServiceRequests(providerUid: string): Promise<FetchRequestsResult> {
  ...
  try {
    ...
    // Fetch consultation IDs for requests that have meeting URLs
    for (const request of requests) {
      if (request.meetingUrl) {
        try {
          ...
          if (!consultsSnapshot.empty) {
            request.consultId = consultsSnapshot.docs[0].id;
          }
        } catch (error) {
          console.error("Error fetching consultation ID for request:", request.id, error);
        }
      }
    }
